...
import {
  GripVertical, Trash2, Plus, PlusSquare, Wand2, X,
  AlignLeft, AlignCenter, AlignRight,
} from "lucide-react";
import { cn } from "@/lib/utils";
import { WorkbookData, TYPE_COLORS } from "@/lib/excel-engine";
import {
  SourceColumn, MappedColumn, ColumnMapping, EMPTY_MAPPING,
...
export type { SourceColumn, MappedColumn, ColumnMapping };
...
function inferDataType(type: SourceColumn["inferredType"]): MappedColumn["dataType"] {
  ...
  const addCol = (blank = false) => {
    update({
      tableColumns: [...mapping.tableColumns, {
        id: makeId(), header: blank ? "Manual Column" : "New Column",
        source: null, width: 100, dataType: "text", alignment: "left", isBlank: blank,
      }],
    });
  };
  ...
  const handleAutoImport = () => {
    ...
    workbook.sheets.forEach(sheet => {
      sheet.columns.forEach(col => {
        ...
        columns.push({
          id: makeId(), header: col.name,
          source: { sheetName: sheet.name, sheetIndex: sheet.index, columnName: col.name, inferredType: col.inferredType },
          width: 100, dataType: inferDataType(col.inferredType),
          alignment: col.inferredType === "number" ? "right" : "left", isBlank: false,
        });
      });
    });
    ...
    <div className="flex flex-col h-full overflow-y-auto">
      <div className="px-5 py-2.5 border-b bg-muted/10 flex items-center justify-between shrink-0">
        <div className="flex items-center gap-3">
          ...
          {mapping.tableColumns.length > 0 && (
            <span className="text-xs text-muted-foreground">{totalMapped} from Excel · {totalBlank} blank</span>
          )}
        </div>
        {workbook && (
          <Button size="sm" variant="outline" className="h-7 gap-1.5 text-xs" onClick={handleAutoImport}>
            <Wand2 className="h-3.5 w-3.5" /> Auto-import columns
          </Button>
        )}
      </div>
      ...
      <div className="flex-1 px-5 py-5 space-y-6">
        <DocumentFieldsSection
          workbook={workbook} mapping={mapping} dragOverDocField={dragOverDocField}
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-0.5">Define columns in the generated PDF table. Drag workbook columns to assign Excel sources.</p>
            </div>
          </div>
            ...
            <>
              <div className="rounded-lg border overflow-hidden">
                <div className="grid items-center bg-muted/50 border-b text-xs font-semibold text-muted-foreground px-2 py-2"
                  ...
                  <span /><span>#</span><span>PDF Column Header</span><span>Source (Excel Column)</span>
                  <span>Type</span><span>Align</span><span>Width</span><span />
                </div>
                {mapping.tableColumns.map((col, idx) => (
                  <TableColumnRow
                    key={col.id} col={col} idx={idx} workbook={workbook}
                    isDragSource={dragRowIdx === idx} isDragOver={dragOverRowIdx === idx} isColDragOver={dragOverColId === col.id}
                    onDragStart={(e) => { e.dataTransfer.setData(DRAG_ROW_KEY, String(idx)); setDragRowIdx(idx); }}
...
function DocumentFieldsSection({ workbook, mapping, dragOverDocField, onDragOver, onDragLeave, onDrop, onChangeField }: {
  workbook: WorkbookData | null; mapping: ColumnMapping; dragOverDocField: string | null;
  onDragOver: (field: string, e: React.DragEvent) => void; onDragLeave: () => void;
  ...
  const fields: Array<{ key: keyof Pick<ColumnMapping, "branchGroupBy" | "branchName" | "state">; label: string; desc: string; required?: boolean }> = [
    { key: "branchGroupBy", label: "Branch Group By", desc: "Groups rows into separate PDFs per branch", required: true },
    { key: "branchName", label: "Branch Name", desc: "Displayed in each PDF header" },
    { key: "state", label: "State / Region", desc: "Displayed in each PDF header" },
  ];
  return (
    <div className="space-y-3">
      <div>
        <h3 className="font-semibold text-sm">Document Fields</h3>
        <p className="text-xs text-muted-foreground mt-0.5">Drag a column from the workbook or use the dropdown to assign these critical fields.</p>
      </div>
          ...
          return (
            <div
              ...
              className={cn(
                ...
                isDragOver ? "border-primary bg-primary/5" : current ? "border-green-500/40 bg-green-50/50 dark:bg-green-900/10" : "border-dashed border-muted-foreground/30"
              )}
              onDragOver={(e) => onDragOver(key, e)} onDragLeave={onDragLeave} onDrop={(e) => onDrop(key, e)}
            >
              ...
              {current ? (
                <div className="flex items-center gap-1.5 mt-2">
                  <span className="h-2 w-2 rounded-full shrink-0" style={{ backgroundColor: workbook?.sheets.find(s => s.name === current.sheetName)?.color ?? "#ccc" }} />
                  <span className="text-xs font-mono font-medium truncate">{current.columnName}</span>
                  <span className="text-xs text-muted-foreground shrink-0">({current.sheetName})</span>
                </div>
              ) : workbook ? (
                <ColumnSelector value={null} workbook={workbook} onChange={(src) => onChangeField(key, src)} placeholder="Select or drag here…" />
              ) : (
...
function TableColumnRow({ col, idx, workbook, isDragSource, isDragOver, isColDragOver, onDragStart, onDragOver, onDragLeave, onDrop, onDragEnd, onColDragOver, onColDragLeave, onColDrop, onUpdate, onRemove }: {
  col: MappedColumn; idx: number; workbook: WorkbookData | null;
  isDragSource: boolean; isDragOver: boolean; isColDragOver: boolean;
  ...
  onColDragOver: (e: React.DragEvent) => void; onColDragLeave: () => void; onColDrop: (e: React.DragEvent) => void;
  onUpdate: (patch: Partial<MappedColumn>) => void; onRemove: () => void;
}) {
  return (
    <div draggable onDragStart={onDragStart} onDragOver={onDragOver} onDragLeave={onDragLeave} onDrop={onDrop} onDragEnd={onDragEnd}
      className={cn("grid items-center border-b last:border-b-0 px-2 py-1.5 text-xs transition-colors", isDragOver && "bg-primary/5", isDragSource && "opacity-40")}
      style={{ gridTemplateColumns: "24px 28px 1fr 200px 88px 68px 68px 32px" }}>
      <span className="text-muted-foreground cursor-grab active:cursor-grabbing flex items-center"><GripVertical className="h-3.5 w-3.5" /></span>
      <span className="text-muted-foreground font-mono">{idx + 1}</span>
      <div className="pr-2">
        <Input value={col.header} onChange={e => onUpdate({ header: e.target.value })} className="h-6 text-xs px-2 w-full" placeholder="Column label" />
      </div>
      <div className={cn("pr-2 rounded transition-colors", isColDragOver && "ring-2 ring-primary ring-offset-1")}
        onDragOver={onColDragOver} onDragLeave={onColDragLeave} onDrop={onColDrop}>
        {col.isBlank || !workbook ? (
          <div className="h-6 px-2 flex items-center rounded border border-dashed border-muted-foreground/30 text-muted-foreground italic text-xs">Blank / manual</div>
        ) : col.source ? (
          <div className="h-6 px-2 flex items-center gap-1.5 rounded bg-muted border cursor-pointer hover:border-muted-foreground/30" onClick={() => onUpdate({ source: null })}>
          ...
          <ColumnSelector value={col.source} workbook={workbook} onChange={(src) => onUpdate({ source: src, isBlank: false, header: col.header || src?.columnName || "" })} placeholder="Drop or select…" />
        )}
      </div>
      <Select value={col.dataType} onValueChange={(v: MappedColumn["dataType"]) => onUpdate({ dataType: v })}>
        <SelectTrigger className="h-6 text-xs mr-2"><SelectValue /></SelectTrigger>
        <SelectContent>
      ...
      <div className="flex items-center gap-0.5 mr-2">
        {(["left", "center", "right"] as const).map(align => (
          <button key={align} onClick={() => onUpdate({ alignment: align })}
            className={cn("h-6 w-6 flex items-center justify-center rounded transition-colors", col.alignment === align ? "bg-primary text-primary-foreground" : "hover:bg-muted text-muted-foreground")}>
            {align === "left" ? <AlignLeft className="h-3 w-3" /> : align === "center" ? <AlignCenter className="h-3 w-3" /> : <AlignRight className="h-3 w-3" />}
          </button>
        ))}
      </div>
      <div className="mr-2">
        <Input type="number" value={col.width} onChange={e => onUpdate({ width: Math.max(30, Number(e.target.value)) })} className="h-6 text-xs px-1.5 w-full" min={30} max={400} />
      </div>
...
export function ColumnSelector({ value, workbook, onChange, placeholder }: {
  value: SourceColumn | null; workbook: WorkbookData; onChange: (src: SourceColumn | null) => void; placeholder?: string;
}) {
  const valueStr = value ? `${value.sheetName}|||${value.columnName}` : "";
  return (
    <Select value={valueStr} onValueChange={(v) => {
      ...
      <SelectTrigger className="h-6 text-xs"><SelectValue placeholder={placeholder ?? "Select column…"} /></SelectTrigger>
      <SelectContent className="max-h-56">
        <SelectItem value="__none__" className="text-xs text-muted-foreground italic">— Not mapped —</SelectItem>
        {workbook.sheets.map(sheet => (
          <SelectGroup key={sheet.name}>
            <SelectLabel className="text-xs flex items-center gap-1.5 py-1">
              <span className="h-1.5 w-1.5 rounded-full" style={{ backgroundColor: sheet.color }} />{sheet.name}
            </SelectLabel>
...
function EmptyTableState({ workbook, onAddCol, onAutoImport }: { workbook: WorkbookData | null; onAddCol: (blank?: boolean) => void; onAutoImport: () => void }) {
  return (
    <div className="rounded-lg border-2 border-dashed border-muted-foreground/25 p-8 text-center">
      ...
      <p className="text-xs text-muted-foreground mb-5">{workbook ? "Auto-import all columns from your workbook, or add them one by one." : "Upload a workbook first, then define your table columns."}</p>
      <div className="flex items-center justify-center gap-2 flex-wrap">
        {workbook && <Button size="sm" className="gap-1.5" onClick={onAutoImport}><Wand2 className="h-4 w-4" /> Auto-import All</Button>}
        <Button size="sm" variant="outline" className="gap-1.5" onClick={() => onAddCol(false)}><Plus className="h-4 w-4" /> Add Column</Button>
        <Button size="sm" variant="outline" className="gap-1.5" onClick={() => onAddCol(true)}><PlusSquare className="h-4 w-4" /> Add Blank Column</Button>
      </div>
    </div>
  );
}
